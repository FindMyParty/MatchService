...
| Segmento | Exemplos |
|---|---|
| quem publicou | `profile`, `match`, `discovery` |
| entidade | `profile`, `swipe`, `match`, `suggestion` |
| ação no passado | `created`, `updated`, `synced` |

Filas seguem `<consumidor>.<publisher>.<entidade>` — prefixo do serviço consumidor concatenado com os dois primeiros segmentos do evento.

### Exchanges e filas consumidas

Todos os nomes são configuráveis por variável de ambiente (ver seção **Variáveis de ambiente**). Os valores abaixo são os padrões.

| Exchange | Tipo | Fila | Binding key | Evento tratado |
|---|---|---|---|---|
| `profile.events` | topic | `match-service.profile.profile` | `profile.profile.*` | Qualquer evento de perfil → upsert local (create/update/sync na mesma fila) |
| `match` | topic | `match-service.match.swipe` | `match.swipe.*` | `match.swipe.created` → lógica de match |
| `suggestion` | topic | `match-service.discovery.suggestion` | `discovery.suggestion.*` | carrega sugestões no Redis — publicado pelo **Discovery Engine** |

Todas as filas têm DLQ associada derivada do nome da fila com sufixo `.dlq`. Mensagens com erro de parse ou processamento recebem `nack(false, false)` e vão para a DLQ.

### Eventos publicados

| Exchange | Routing key | Quem consome | Quando |
|---|---|---|---|
| `match` | `match.swipe.created` | match-service (si mesmo) | HTTP POST /swipes — SwipeUseCase publica e o subscriber consome |
...
**Relação com o Discovery Engine:** o Discovery Engine deve publicar sugestões com routing key `discovery.suggestion.created` na exchange `suggestion` e consumir `match.match.created` para fechar o ciclo.

**Compatibilidade com o ProfileService:** o ProfileService publica create/update/resync com a **mesma routing key** (`PROFILE_EVENT_ROUTING_KEY`, padrão `profile.profile.updated`) e sem campo discriminador de tipo — o MatchService faz `upsert` em todos os casos. Dois vínculos de configuração não podem ser quebrados:
- `RABBITMQ_PROFILE_EXCHANGE` no MatchService deve ser `profile.events` (hardcoded no ProfileService).
- `PROFILE_EVENT_ROUTING_KEY` no ProfileService deve seguir o padrão `profile.profile.<sufixo>` para ser capturado pelo binding `profile.profile.*`.
...
| `REDIS_TTL` | não | `86400` | TTL dos sets Redis em segundos |
...
| `RABBITMQ_PROFILE_EXCHANGE` | não | `profile.events` | Exchange de eventos de perfil (ProfileService) |
| `RABBITMQ_PROFILE_QUEUE` | não | `match-service.profile.profile` | Fila de eventos de perfil |
| `RABBITMQ_PROFILE_BINDING_KEY` | não | `profile.profile.*` | Binding key da fila de perfil |
| `RABBITMQ_SWIPE_EXCHANGE` | não | `match` | Exchange de swipes |
| `RABBITMQ_SWIPE_QUEUE` | não | `match-service.match.swipe` | Fila de swipes |
| `RABBITMQ_SWIPE_BINDING_KEY` | não | `match.swipe.*` | Binding key da fila de swipe |
| `RABBITMQ_SUGGESTION_EXCHANGE` | não | `suggestion` | Exchange de sugestões (Discovery Engine) |
| `RABBITMQ_SUGGESTION_QUEUE` | não | `match-service.discovery.suggestion` | Fila de sugestões |
| `RABBITMQ_SUGGESTION_BINDING_KEY` | não | `discovery.suggestion.*` | Binding key da fila de sugestões |
